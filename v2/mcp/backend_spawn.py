...
The `.env` file stores the full API base URL as ``TRACKEROO_API_URL=<url>``
(e.g. ``TRACKEROO_API_URL=http://127.0.0.1:8787``). This means the value can
point at any host — local or remote — without changing the discovery protocol.
The legacy ``TRACKEROO_PORT=<port>`` key (written by older app versions) is
still accepted as a fallback so existing project folders keep working.
...
def _read_api_url(folder: Path) -> str | None:
    """Read the backend's full API base URL from the project's .env file.

    Understands both the current ``TRACKEROO_API_URL`` key (written by app v2
    and later) and the legacy ``TRACKEROO_PORT`` key (written by older app
    versions) so that existing projects continue to work after an update.
    Returns ``None`` if neither key is present or the file doesn't exist.
    """
    try:
        env = _parse_env_file(_env_file(folder).read_text())
    except FileNotFoundError:
        return None
    if url := env.get("TRACKEROO_API_URL"):
        return url.rstrip("/")
...
def ensure_backend_running(folder: Path) -> str:
    ...
    api_url = _read_api_url(folder)
    if api_url is not None and _health_ok(api_url):
        return api_url

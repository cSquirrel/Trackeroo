...
def _read_port(folder: Path) -> int | None:
    try:
...
def _health_ok(port: int) -> bool:
    try:
        return httpx.get(f"http://localhost:{port}/api/health", timeout=1.0).is_success
...
def ensure_backend_running(folder: Path) -> str:
    ...
    with open(_lock_file(folder), "a+") as lockfile:
        ...
        try:
            ...
            if not _wait_for_health(new_port):
                proc.kill()
                raise RuntimeError(
                    f"Trackeroo backend for '{folder}' did not become healthy in time."
                )
            _env_file(folder).write_text(f"TRACKEROO_PORT={new_port}\n")
            return f"http://localhost:{new_port}"
        finally:

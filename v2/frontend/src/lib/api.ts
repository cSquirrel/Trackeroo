...
// In the Tauri app the webview is served from Tauri's own asset scheme, not
// from the backend, so relative `/api` fetches would never reach the sidecar.
// Default to the sidecar's absolute URL; override via VITE_API_BASE_URL.
